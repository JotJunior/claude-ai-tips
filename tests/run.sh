...
#
# Convencao de cobertura (FR-009): um script em global/skills/<skill>/scripts/
# <nome>.sh corresponde a tests/test_<nome>.sh. Test files internos (sem
# script correspondente em global/skills/) como test_smoke e test_harness sao
# excluidos do check de orfaos.
#
...
# Imprime caminho absoluto de cada tests/test_*.sh, filtrado por PATTERN
# (substring sobre o path) se fornecido. Ordena para determinismo.
_find_test_files() {
  _filter="${1:-}"
  _all=$(find "$TESTS_ROOT" -maxdepth 1 -name 'test_*.sh' -type f 2>/dev/null | sort)
  if [ -z "$_filter" ]; then
    printf '%s\n' "$_all"
    return 0
  fi
...
# _find_scripts
# Imprime caminho absoluto de cada .sh em global/skills/<any>/scripts/.
_find_scripts() {
  find "$REPO_ROOT/global/skills" -type f -path '*/scripts/*.sh' 2>/dev/null | sort
}
...
'
  ...
  for _script in $_scripts; do
    [ -z "$_script" ] && continue
    _sb=$(_script_basename "$_script")
    _expected="$TESTS_ROOT/test_$_sb.sh"
    if [ ! -f "$_expected" ]; then
...
mode_check_coverage() {
  ...
  printf 'Cobertura de testes para scripts em global/skills/**/scripts/\n\n'
